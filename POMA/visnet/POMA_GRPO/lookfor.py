...
# 导入配置与模型 (这里假设 model.py 和 data.py 在 visnet 目录下)
...
def _isolated_base_worker(target_csv, sup_paths, gpu_id, g_mean, g_std, return_dict):
    ...
    try:
        ...
        model = ViSNet(
            hidden_channels=128, num_layers=6, num_heads=8, num_rbf=32, 
            mean=dm.mean.to(device), std=dm.std.to(device), num_tasks=1
        ).to(device)
...
def worker_fast_proxy(s_idx, action_vec, gpu_id, paths, target_csv, sup_paths, base_model_path, mae_ref, g_mean, g_std, return_dict):
    ...
    try:
        ...
        model = ViSNet(hidden_channels=128, num_layers=6, num_heads=8, num_rbf=32, 
                       mean=dm.mean.to(device), std=dm.std.to(device), num_tasks=1).to(device)
...
def run_meta_grpo():
    ...
    for s_idx in range(40):
        ...
        p_base = mp.Process(target=_isolated_base_worker, args=(tmp_target_csv, cfg.sup_source_paths, 2, g_mean, g_std, ret_base))
        p_base.start()
        p_base.join()
        ...
        while pending_tasks or any(gpu_slots.values()):
            for gid in [0, 1]:
                if gpu_slots[gid] is None and pending_tasks:
                    target_mod = 1 if gid == 0 else 0 
                    task_idx = next((i for i, t in enumerate(pending_tasks) if t % 2 == target_mod), -1)
                    
                    if task_idx != -1:
                        tid = pending_tasks.pop(task_idx)
                        p = mp.Process(target=worker_fast_proxy, args=(tid, actions_np_main[tid], gid, paths, tmp_target_csv, cfg.sup_source_paths, base_path, mae_ref, g_mean, g_std, results_map))
                        p.start()
                        gpu_slots[gid] = (tid, p)
