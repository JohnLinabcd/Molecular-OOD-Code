...
# ... [保留 PART 1, PART 2, PART 3 (ViSNetBlock, GatedEquivariantBlock) 所有基础类，和之前一样] ...
# =============================================================================
# PART 1: 基础数学与几何组件
# =============================================================================
...
class ExpNormalSmearing(torch.nn.Module):
    def __init__(self, cutoff: float = 5.0, num_rbf: int = 128, trainable: bool = True) -> None:
        super().__init__()
        self.cutoff = cutoff
...
class Sphere(torch.nn.Module):
    ...
    def forward(self, edge_vec: Tensor) -> Tensor:
        return self._spherical_harmonics(self.lmax, edge_vec[..., 0], edge_vec[..., 1], edge_vec[..., 2])
...
class VecLayerNorm(torch.nn.Module):
    def __init__(self, hidden_channels: int, trainable: bool, norm_type: Optional[str] = 'max_min') -> None:
        super().__init__()
    ...
    def max_min_norm(self, vec: Tensor) -> Tensor:
        ...
        if (dist == 0).all(): return torch.zeros_like(vec)
    ...
    def forward(self, vec: Tensor) -> Tensor:
        if vec.size(1) == 3:
            if self.norm_type == 'max_min': vec = self.max_min_norm(vec)
            ...
            return vec * self.weight.unsqueeze(0).unsqueeze(0)
        raise ValueError(f"'{self.__class__.__name__}' only support 3 or 8 channels (got {vec.size(1)})")
...
class Distance(torch.nn.Module):
    ...
    def forward(self, pos: Tensor, batch: Tensor) -> Tuple[Tensor, Tensor, Tensor]:
        edge_index = radius_graph(pos, r=self.cutoff, batch=batch, loop=self.add_self_loops, max_num_neighbors=self.max_num_neighbors)
...
class NeighborEmbedding(MessagePassing):
    def __init__(self, hidden_channels: int, num_rbf: int, cutoff: float, max_z: int = 100) -> None:
    ...
    def forward(self, z: Tensor, x: Tensor, edge_index: Tensor, edge_weight: Tensor, edge_attr: Tensor) -> Tensor:
        mask = edge_index[0] != edge_index[1]
        if not mask.all():
...
class ViS_MP(MessagePassing):
    def __init__(self, num_heads: int, hidden_channels: int, cutoff: float, vecnorm_type: Optional[str], trainable_vecnorm: bool, last_layer: bool = False) -> None:
        super().__init__(aggr='add', node_dim=0)
        if hidden_channels % num_heads != 0: raise ValueError("hidden_channels must be divisible by num_heads")
    ...
    def forward(self, x: Tensor, vec: Tensor, edge_index: Tensor, r_ij: Tensor, f_ij: Tensor, d_ij: Tensor) -> Tuple[Tensor, Tensor, Optional[Tensor]]:
        x = self.layernorm(x)
    ...
    def message(self, q_i: Tensor, k_j: Tensor, v_j: Tensor, vec_j: Tensor, dk: Tensor, dv: Tensor, r_ij: Tensor, d_ij: Tensor) -> Tuple[Tensor, Tensor]:
    ...
    def aggregate(self, features: Tuple[Tensor, Tensor], index: Tensor, ptr: Optional[torch.Tensor], dim_size: Optional[int]) -> Tuple[Tensor, Tensor]:
        x, vec = features
...
class ViS_MP_Vertex(ViS_MP):
    ...
    def reset_parameters(self):
        ...
        if not self.last_layer:
            if hasattr(self, 't_src_proj'): torch.nn.init.xavier_uniform_(self.t_src_proj.weight)
            if hasattr(self, 't_trg_proj'): torch.nn.init.xavier_uniform_(self.t_trg_proj.weight)
...
class ViSNetBlock(torch.nn.Module):
    def __init__(self, lmax: int = 1, vecnorm_type: Optional[str] = None, trainable_vecnorm: bool = False, num_heads: int = 8, num_layers: int = 6, hidden_channels: int = 128, num_rbf: int = 32, trainable_rbf: bool = False, max_z: int = 100, cutoff: float = 5.0, max_num_neighbors: int = 32, vertex: bool = False) -> None:
        super().__init__()
        self.lmax = lmax
        ...
        vis_mp_kwargs = dict(num_heads=num_heads, hidden_channels=hidden_channels, cutoff=cutoff, vecnorm_type=vecnorm_type, trainable_vecnorm=trainable_vecnorm)
    ...
    def reset_parameters(self):
        ...
        for layer in self.vis_mp_layers: layer.reset_parameters()
...
class GatedEquivariantBlock(torch.nn.Module):
    def __init__(self, hidden_channels: int, out_channels: int, intermediate_channels: Optional[int] = None, scalar_activation: bool = False) -> None:
        super().__init__()
        self.out_channels = out_channels
        if intermediate_channels is None: intermediate_channels = hidden_channels
...
class EquivariantFeatureHead(torch.nn.Module):
    def __init__(self, hidden_channels: int) -> None:
        ...
        self.output_network = torch.nn.ModuleList([
            GatedEquivariantBlock(hidden_channels, hidden_channels, scalar_activation=True),
            GatedEquivariantBlock(hidden_channels, hidden_channels, scalar_activation=True),
        ])
    def reset_parameters(self):
        for layer in self.output_network: layer.reset_parameters()
    def forward(self, x: Tensor, v: Tensor) -> Tensor:
        for layer in self.output_network: x, v = layer(x, v)
        return x 
...
class ViSNet(torch.nn.Module):
    def __init__(
        ...
        self.representation_model = ViSNetBlock(
            ...
            num_heads=num_heads, num_layers=num_layers, hidden_channels=hidden_channels,
            num_rbf=num_rbf, trainable_rbf=trainable_rbf, max_z=max_z, cutoff=cutoff,
            max_num_neighbors=max_num_neighbors, vertex=vertex,
        )

        self.feature_extractor = EquivariantFeatureHead(hidden_channels)
        ...
        if mean is None: mean = torch.zeros(1, num_tasks)
        if std is None: std = torch.ones(1, num_tasks)
    ...
    def forward(self, z: Tensor, pos: Tensor, batch: Tensor, sub_batch: Tensor) -> Tuple[Tensor, Tensor]:
        # 1. 提取原子级特征
        x_atom, v_atom = self.representation_model(z, pos, batch)
        x_atom_feat = self.feature_extractor(x_atom, v_atom)

        # 2. ✨ 子结构特征聚合 (专门用于 CORAL/MMD 对齐) ✨
        x_sub_feat = scatter(x_atom_feat, sub_batch, dim=0, reduce=self.reduce_op)
        ...
        # 返回: [Batch, 2] 预测值, [N_subs, 128] 子结构特征
